...
export class BusService {
  ...
  async findAll(
    ...
    const organizations = await this.organizationService.findAll(-1, -1, {
      owner: user['_id'],
    });
    const organizationIds = organizations.results.map(
      (organization) => organization._id,
    );
    if (organization in organizationIds) {
      return this.busesService.findAll(page, limit, {
  ...
  async update(
    request: Request,
    id: string,
    updateBusDto: Record<string, any>,
  ) {
